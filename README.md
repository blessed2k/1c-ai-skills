...
Один плагин `1c-dev` с шестью скиллами:

| Команда | Что делает |
|---|---|
| `/1c-dev:setup` | Задаёт несколько вопросов и записывает настройки под ваш проект: как вносятся правки, куда класть спеки, чем проверять код |
...
`grill` и `vanessa-pitfalls` агент может вызвать сам, когда видит подходящую ситуацию. Остальные
запускаются только вашей командой.
...
Обновление: `/plugin marketplace update 1c-ai-skills`.
...
поведенческие тест-кейсы из `plugins/1c-dev/evals` через `claude plugin eval`: это платные
прогоны модели на вашей учётной записи. `forbidden.mjs` проверяет публикацию по приватному
списку строк мейнтейнера и без этого списка не запускается.
